...
const getDriverDocumentStatus = async (pool, driverId) => {
  const result = await pool.query(
    `SELECT document_type, s3_key, status, uploaded_at, verified_at FROM driver_documents WHERE driver_id = $1`,
    [driverId]
  );
  const uploaded = {};
  result.rows.forEach(doc => {
    uploaded[doc.document_type] = {
      key: doc.s3_key, status: doc.status, uploadedAt: doc.uploaded_at, verifiedAt: doc.verified_at,
    };
  });
  const status = {};
